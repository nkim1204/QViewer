...
void MainWindow::on_actionOpen_EnvMap_triggered()
{
	if(_scene)
	{
		QString filename = 
			myFileDialog(QFileDialog::AcceptOpen, "Open Scene", 
			"Env Map (*.hdr *.png *.jpg *.jpeg)", 
			_last_scene_dir );

		if (!filename.isNull())
		{
			_scene->loadEnvMap(filename);
		}
	}
}
...
void MainWindow::setupFileMenu()
{
	...
	QAction* openEnvMapAction = new QAction(tr("&Open Env. Map"), 0);
    openEnvMapAction->setShortcut(QKeySequence(tr("Ctrl+E")));
    connect(openEnvMapAction, SIGNAL(triggered()), 
        this, SLOT(on_actionOpen_EnvMap_triggered()));
...
void MainWindow::on_actionOpen_Scene_triggered()
{
    QString filename = 
        myFileDialog(QFileDialog::AcceptOpen, "Open Scene", 
        "Scenes (*." + Scene::fileExtension() + " *.off *.obj *.ply *.3ds)", 
        _last_scene_dir );
